...
def send_report_email(recipient, report_text, report_type, role, dataset_name):
    """Send a formatted HTML report email via SMTP credentials in secrets."""
    try:
        ...
        return False, "Email delivery not configured. Contact the platform admin."

    html = f"""
    <html><body style="font-family:Inter,Arial,sans-serif;color:#1E293B;max-width:660px;margin:auto;padding:0;">
      <div style="background:#2563EB;padding:22px 28px;border-radius:10px 10px 0 0;">
        <h2 style="color:#fff;margin:0;font-size:1.3rem;font-weight:700;">Nixara Report</h2>
        <p style="color:#BFDBFE;margin:5px 0 0;font-size:0.82rem;">{report_type} &nbsp;·&nbsp; {role} &nbsp;·&nbsp; {dataset_name}</p>
      </div>
      <div style="background:#F8F9FC;padding:24px 28px;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 10px 10px;">
        <pre style="white-space:pre-wrap;font-family:Inter,Arial,sans-serif;font-size:0.87rem;line-height:1.75;color:#1E293B;">{report_text}</pre>
      </div>
      <p style="font-size:0.70rem;color:#94A3B8;margin-top:14px;text-align:center;">
        Generated by <a href="https://ai-dashboard-copilot.streamlit.app" style="color:#2563EB;text-decoration:none;">Nixara</a>
        &nbsp;·&nbsp; AI-powered business intelligence
      </p>
    </body></html>
    """
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Nixara — {report_type} ({role})"
    msg["From"] = f"Nixara <{smtp_user}>"
    msg["To"] = recipient
    ...
    try:
        ...
        return True, "✓ Report sent to " + recipient
...
To remove your key at any time, untick "Remember key in this browser".
    ...
    st.markdown("---")
    with st.expander("🏢  Enterprise Branding"):
        st.caption("Personalise Nixara with your company identity.")
        wb_name  = st.text_input("Company name", placeholder="e.g. Acme Corp", key="wb_name")
        wb_color = st.color_picker("Brand accent colour", "#2563EB", key="wb_color")
        wb_logo  = st.text_input(
            "Logo URL  (optional)",
            placeholder="https://yourcompany.com/logo.png",
            key="wb_logo",
            help="Link to a PNG or SVG logo hosted on your website. No file upload needed.",
        )
        if wb_name:
            st.success("✓ White-label active", icon="✅")
...
# ---------------------------------------------------
# APP HEADER — default or white-label
# ---------------------------------------------------

_wb_name  = st.session_state.get("wb_name",  "")
_wb_color = st.session_state.get("wb_color", "#2563EB")
_wb_logo  = st.session_state.get("wb_logo",  "")

if _wb_name:
    # ── White-label header ──────────────────────────────────────────────────
    _logo_html = (
        f'<img src="{_wb_logo}" style="height:42px;object-fit:contain;'
        f'margin-bottom:10px;display:block;max-width:200px;" alt="{_wb_name} logo">'
        if _wb_logo else ""
    )
    st.markdown(f"""
        <div style="border-bottom:1.5px solid #E2E8F0;padding-bottom:1.25rem;margin-bottom:2rem;">
            {_logo_html}
            <p style="font-size:3.0rem;font-weight:700;letter-spacing:-0.03em;
                      color:#1E293B;margin:0;line-height:1.1;">{_wb_name}</p>
            <p style="font-size:0.82rem;color:#94A3B8;margin:6px 0 10px;font-weight:400;">
                Intelligence Hub &nbsp;·&nbsp;
                Powered by <strong style="color:{_wb_color};">Nixara</strong>
            </p>
            <div style="height:3px;width:52px;background:{_wb_color};
    ...
    """, unsafe_allow_html=True)
    # Inject brand colour overrides
    if _wb_color != "#2563EB":
        st.markdown(f"""
        <style>
        .accent-bar {{ background: {_wb_color} !important; }}
        .stButton > button {{ background: {_wb_color} !important; border-color: {_wb_color} !important; }}
        a {{ color: {_wb_color} !important; }}
        strong {{ color: inherit; }}
        </style>
        """, unsafe_allow_html=True)
else:
    # ── Default Nixara header ───────────────────────────────────────────────
    st.markdown("""
        <div style="border-bottom: 1.5px solid #E2E8F0; padding-bottom: 1.25rem; margin-bottom: 2rem;">
            ...
            if not decision.strip():
                ...
                for tab, report_type in zip(
                    ...
                    with tab:
                        ...
                        _email_key = f"email_addr_{report_type}"
                        _sent_key  = f"email_sent_{report_type}"
                        if _email_key not in st.session_state:
                            st.session_state[_email_key] = ""

                        em_col1, em_col2 = st.columns([3, 1])
                        with em_col1:
                            st.session_state[_email_key] = st.text_input(
                                "📧 Email this report",
                                value=st.session_state[_email_key],
                                placeholder="you@company.com",
                                label_visibility="collapsed",
                                key=f"email_input_{report_type}",
                            )
                        with em_col2:
                            if st.button("Send", key=f"send_{report_type}", width='stretch'):
                                _addr = st.session_state[_email_key].strip()
                                if _addr and "@" in _addr:
                                    _ds_name = uploaded_file.name if uploaded_file else "dataset"
                                    _ok, _msg = send_report_email(
                                        _addr, report_text, report_type, who, _ds_name
                                    )
                                    st.session_state[_sent_key] = (_ok, _msg)
                                else:
                                    st.session_state[_sent_key] = (False, "Enter a valid email address.")

                        if _sent_key in st.session_state:
                            _ok, _msg = st.session_state[_sent_key]
                            if _ok:
                                st.markdown(
                                    f'<p style="font-size:0.75rem;color:#2E7D52;margin-top:4px;">{_msg}</p>',
                                    unsafe_allow_html=True,
                                )
                            else:
                                st.markdown(
                                    f'<p style="font-size:0.75rem;color:#C0392B;margin-top:4px;">{_msg}</p>',
                                    unsafe_allow_html=True,
                                )
